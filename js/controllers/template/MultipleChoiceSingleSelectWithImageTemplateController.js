...
var MultipleChoiceSingleSelectWithImageTemplateController = function(){
    ...
    this.loadUI = function(){
        ...
        for(var i=0;i<radioInputLength;i++){
            radioInput += '<div class="radio"><input type="radio" class="radioColor" name="radioOption" id="radio_' + (i + parseInt('1')) + '" value="' + jsonData.pageContent.options[i] + '" /><label for="radio_' + (i + parseInt('1')) + '">' + jsonData.pageContent.options[i] + '</label></div>';
        }
		
		$('#singleChoice').html('');
        $('#singleChoice').html(radioInput);

		if((jsonData.pageContent.image != undefined) && (jsonData.pageContent.image != "")){
			$("#main_img").attr("src",""+baseURL+"assets/images/"+jsonData.pageContent.image+"");
		} 
